function handleFormSubmit(event) {
  ...
  function fetchAndDisplayUsers() {
    ...
    axios.get("`http://51.20.67.98:4000/expense/get-expense",{headers :{"Authorization" :token}})
      .then((response) => {
...
return JSON.parse(jsonPayload);
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const page=1
    axios.get(`http://51.20.67.98:4000/products?page=${page}`)
    .then(res =>{
  ...
  function deleteUser(userId) {
    const token=localStorage.getItem("token")
    axios.delete(`http://51.20.67.98:4000/expense/delete-expense/${userId}`,{headers :{"Authorization" :token}})
      .then(response => {
...
function download(){
  const token=localStorage.getItem("token")
  axios.get('`http://51.20.67.98:4000/user/download', { headers: {"Authorization" : token} })
  .then((response) => {
...
function totaldownload(){
  const token=localStorage.getItem("token")
  axios.get('`http://51.20.67.98:4000/user/totaldownloads', { headers: {"Authorization" : token} })
  .then((response) => {
      if(response.status === 200){
